...
# ─── CONSTANTS ─────────────────────────────────────────────────────────────────
A        = 53.99302        # intercept fungsi permintaan (skala)
B        = 1.136737        # kemiringan (absolut)
MC_BASE  = 283817.2        # MC rata-rata (Rp/ton)
HBA_BASE = 1.9375e6        # HBA rata-rata 2024 (Rp/ton)
CP_BASE  = 863888320.0     # choke price (Rp/ton)
MUC0_BASE = 15163.0        # MUC awal (Rp/ton)
R_BASE   = 0.05            # discount rate
T_STAR_BASE = 114.12       # T* baseline
...
PLOT_STYLE = dict(
    ...
    legend=dict(bgcolor="white", bordercolor="#e9ecef", borderwidth=1,
                font=dict(size=11)),
)
...
def calc_market(hba_m, mc_m, n):
    """
    hba_m, mc_m : harga dalam satuan juta Rp/ton
    Konversi ke skala model: P_model = HBA_juta / HBA_juta_baseline * A
    Q fungsi permintaan dalam juta ton
    """
...
<div class="page-header">
  <h1>⛏️ Analisis Intertemporal Batubara — PT Mitrabara Adiperdana Tbk</h1>
  <p>Dampak harga, diskonto, dan marginal user cost terhadap produksi, stok, dan umur cadangan &nbsp;·&nbsp;
     Tiga struktur pasar &nbsp;·&nbsp; Green Paradox &nbsp;·&nbsp;
     Arif Hamdani · Bambang Karta Wijaya · Moh Bayu Mustofa &nbsp;·&nbsp; UNISBA 2026</p>
</div>
""", unsafe_allow_html=True)

tab0, tab1, tab2, tab3 = st.tabs(["📊 Dashboard", "🏭 Struktur Pasar", "⏳ Efisiensi Dinamis", "🌍 Green Paradox"])
...
with tab0:
    ...
    # ── MC per tahun dengan highlight negatif ─────────────────────────────────
    ...
    st.markdown("""<div class="insight-box">
        💡 <strong>Catatan MC negatif:</strong> Nilai MC negatif pada 2018, 2022, dan 2023 merupakan
        artefak metodologi dari pendekatan <em>incremental COGS</em> — bukan cerminan biaya aktual.
        Terjadi karena COGS turun sementara volume produksi juga turun, menghasilkan selisih negatif.
        Rata-rata MC Rp 283.817/ton digunakan sebagai konstanta model.
    </div>""", unsafe_allow_html=True)

# ═══════════════════════════════════════════════════════════════════════════════
# TAB 1 — STRUKTUR PASAR
# ═══════════════════════════════════════════════════════════════════════════════
with tab1:
...
<div class="market-card pc">
  ...
  <span class="badge badge-g">P = MC · Efisien</span>
</div>""", unsafe_allow_html=True)
...
<div class="market-card oli">
  ...
  <span class="badge badge-y">Antara keduanya</span>
</div>""", unsafe_allow_html=True)
  ...
  <span class="badge badge-r">P > MC · Inefisien</span>
    ...
    if margin > 0:
        msg = (f"Margin keuntungan = Rp {margin:.2f} jt/ton. "
               ...
               f"karena Q* tertinggi. Monopoli menghemat cadangan ({life_mono:.1f} tahun) "
               f"tetapi menciptakan DWL = {m['dwl_mono']:.3f} — kerugian kesejahteraan masyarakat. "
               f"Oligopoli (n={n_firms}) berada di tengah: umur cadangan {life_oli:.1f} tahun, "
               f"DWL = {m['dwl_oli']:.3f}. Semakin banyak perusahaan (n→∞), "
               f"oligopoli mendekati persaingan sempurna.")
        st.markdown(f'<div class="insight-box">💡 <strong>Kesimpulan:</strong> {msg}</div>',
                    unsafe_allow_html=True)
    else:
    ...
    st.markdown('<div class="section-label">Kurva permintaan & ekuilibrium</div>', unsafe_allow_html=True)
    q_range = np.linspace(0, A / B * 1.05, 300)
    p_range = A - B * q_range

    fig = go.Figure()

    # Surplus konsumen (persaingan sempurna)
    ...
    fig.add_trace(go.Scatter(
        ...
        mode="markers", marker=dict(size=12,
            color=["#1D9E75", "#BA7517", "#A32D2D"],
            symbol=["circle", "diamond", "square"],
            line=dict(color="white", width=2)),
        name="Ekuilibrium",
        text=["PC", f"Oli (n={n_firms})", "Mono"],
        textposition="top center"
    ))

    # Garis vertikal ekuilibrium
    for q_eq, p_eq, col, lbl in [
        (m["q_pc"],   m["p_pc"],   "#1D9E75", "PC"),
        (m["q_oli"],  m["p_oli"],  "#BA7517", f"Oli"),
        (m["q_mono"], m["p_mono"], "#A32D2D", "Mono"),
    ]:
        if q_eq > 0:
    ...
    fig.update_layout(title="Kurva Permintaan & Posisi Ekuilibrium Tiga Struktur Pasar",
                      ...
                      yaxis_title="P (harga, skala model)", height=380, **PLOT_STYLE)
    ax(fig)
    ...
    for col_i, v in enumerate(vals, 1):
        fig_bar.add_trace(go.Bar(x=labels3, y=v, marker_color=colors3,
...
# ═══════════════════════════════════════════════════════════════════════════════
# TAB 2 — EFISIENSI DINAMIS (HOTELLING)
# ═══════════════════════════════════════════════════════════════════════════════
with tab2:
    ...
    # ── Metrik utama ──────────────────────────────────────────────────────────
    st.markdown('<div class="section-label">Hasil kalkulasi</div>', unsafe_allow_html=True)
    m1, m2, m3, m4, m5 = st.columns(5)

    with m1:
        st.markdown(f"""<div class="metric-card">
            <span class="num num-blue">{t_star:.1f}</span>
    ...
    direction = "lebih cepat" if t_star < base_t else "lebih lambat"
    st.markdown(f"""<div class="insight-box">
        💡 <strong>Interpretasi:</strong> Dengan diskonto r = {r*100:.1f}%, cadangan optimal habis
        dalam <strong>{t_star:.1f} tahun</strong> (~{int(2025+t_star)}).
        {"Lebih cepat" if t_star < base_t else "Lebih lambat"} dari baseline r=5%
        ({base_t:.0f} tahun). MUC tumbuh eksponensial dari Rp {muc0/1000:.0f} ribu/ton
        menjadi Rp {muc50/1e6:.1f} jt/ton pada t=50.
        {"Diskonto tinggi mendorong eksploitasi lebih cepat karena nilai masa depan lebih rendah."
         if r > R_BASE else
         "Diskonto rendah memberi insentif konservasi — generasi mendatang dinilai setara."}
    </div>""", unsafe_allow_html=True)

    # ── Grafik MUC ────────────────────────────────────────────────────────────
    st.markdown('<div class="section-label">Pertumbuhan MUC & titik T*</div>', unsafe_allow_html=True)
    t_max = min(t_star * 1.5, 300) if t_star else 200
    ...
    # ── Sensitivitas T* ───────────────────────────────────────────────────────
    st.markdown('<div class="section-label">Sensitivitas T* terhadap tingkat diskonto</div>',
                unsafe_allow_html=True)
    ...
    with cr:
        ...
        st.markdown("""<div style="font-size:0.78rem;color:#6c757d;margin-top:8px">
            Diskonto tinggi → T* pendek → eksploitasi lebih cepat.
            Diskonto rendah = menghargai generasi mendatang lebih tinggi.
        </div>""", unsafe_allow_html=True)
...
with tab3:
    ...
    st.markdown('<div class="section-label">Parameter skenario</div>', unsafe_allow_html=True)
    ...
    base_prod = 3.0  # juta ton per tahun (rata-rata MBAP)
    ...
    # Emisi (asumsi 2.5 ton CO₂ per ton batubara)
    ...
    # ── Metrik ────────────────────────────────────────────────────────────────
    st.markdown('<div class="section-label">Dampak emisi kumulatif (2025–2044)</div>',
                unsafe_allow_html=True)
    ...
    if delta_em > 5:
        st.markdown(f"""<div class="danger-box">
            ...
            berlawanan dengan tujuan kebijakan lingkungan.
            Solusi: pajak berlaku segera dan bertahap, bukan diumumkan jauh hari.
        </div>""", unsafe_allow_html=True)
    elif delta_em < -5:
        st.markdown(f"""<div class="success-box">
            ✅ Kebijakan efektif — emisi turun {abs(delta_em):.1f} Mt CO₂ ({pct_change:.1f}%).
            Pajak cukup besar dan/atau implementasi cukup cepat sehingga tidak memicu paradox.
        </div>""", unsafe_allow_html=True)
    else:
        st.markdown(f"""<div class="warn-box">
            ⚡ Dampak marginal — paradox kecil. Pertimbangkan menaikkan pajak atau
            memperpendek lag implementasi untuk efek yang lebih signifikan.
        </div>""", unsafe_allow_html=True)

    # ── Grafik produksi ───────────────────────────────────────────────────────
    st.markdown('<div class="section-label">Volume produksi: baseline vs skenario Green Paradox</div>',
                unsafe_allow_html=True)
    ...
    st.markdown("""<div class="insight-box">
        💡 <strong>Mekanisme Green Paradox (Sinn, 2008):</strong>
        ...
        tujuan kebijakan. <strong>Tiga solusi berbasis bukti:</strong>
        (1) Pajak berlaku segera dengan kenaikan bertahap (<em>escalating carbon price</em>);
        (2) Kuota produksi absolut berbasis anggaran karbon;
        (3) Moratorium ekspansi konsesi baru.
    </div>""", unsafe_allow_html=True)
    ...
    st.markdown("""<div style="font-size:0.78rem;color:#adb5bd;text-align:center;padding:8px 0">
        ...
        Universitas Islam Bandung · 2026
    </div>""", unsafe_allow_html=True)